...
def computePlaneCoeffs(heavyAtoms, debug=0):
...
def computeHydCoordsSp2( heavyAtoms, clashOccurs, debug =0):
    ''' 
...
#    ###################################################################################################################
def addLPtoRes(res, LPname, lpCoords, countSerial, LPcoordsInfo, debug=0):
    '''
...
def placeLP_BB(structure, lastSerial, debug = 0):
    '''
    ...
    for model in structure.child_list:
        for chain in model.child_list: 
            ...
            for res in chain.child_list: 
                ...
                if(myRes.isValidAminoAcid()==1):

                    resACE = False
                    count+=1
                    
                    #Mark if the first residue is an ACE
                    if(count==1 and myRes.isValidAminoAcid()==1 ):
                        resACE = res.resname =='ACE'
                     
                    if(myRes.isValidAminoAcid()==1 and resACE == False and res.resname != 'NME'): 
...
def placeHydrogens_BB(structure, lastSerial, debug = 0):
    '''
    ...
    for model in structure.child_list:
        for chain in model.child_list:
            ...
            for atom in chain.get_atoms():
                ...
                if(mra.myResidue(a.get_parent()).isValidAminoAcid()==1): 
                    if(a.get_name() == 'N'):
...
def placeHydrogens_GLN(res, lastSerial, debug = 0):
...
def placeHydrogens_HID(res, lastSerial, debug = 0):
...
def placeHydrogens_HIP(res, lastSerial, debug = 0):
...
def addSP2SideChainHydrogens(structure,lastSerial, debug =0):
    ...
    res_dict = {'ARG': placeHydrogens_ARG, 'ASN':placeHydrogens_ASN, 'GLN': placeHydrogens_GLN, 'TYR': placeHydrogens_TYR, 'TRP': placeHydrogens_TRP, 'HID': placeHydrogens_HID, 'HIE': placeHydrogens_HIE, 'HIP': placeHydrogens_HIP}
...
def addSP2SideChainLPs(structure, lastSerial, debug =0):
