...
def setupHIS(HISres, structure, HIStype = None, debug =0):
...
def setupRotamerInStruct(res, structure, placeHyd = True, placeLP = True, debug = 0):
    '''
...
def computeEnergyForGivenState(resState, chV_level, con):
    ...
    #Find list side chain active actoms, i.e donor/acceptor
    LOAAresState = mra.myResidue(resState).get_unknownResActiveAtoms() 
    enSumTotal = 0
    enSumForHvys = []
...
def evaluate_degenerateCases( unknownRes,structure, S, energyArray, sortedEnArray, unknownResMod, changeVal, skipVal, collectData, skipResInfo, debug = 0):
...
###############################ADD to check if one of the acceptor pos is SER/THR######################################################################
        ...
        LOAA_unknownResR = myHIPR.get_unknownResActiveAtoms()
