...
def get_close_atom_list(targAtom, givenList, debug=0):

    ''' 
    objective: Get a list of close atom for a target atom
    Input: Target atom which is a my_atom type object.
    ...
    if(not givenList): 
        stp.append_to_log("No search List provided!!-STOP! \n", debug=0)
        os._exit(0)
...
def get_close_atom_distance_info(atomCurr, close_atoms, debug = 0):
    '''
...
def get_all_close_atom_info_for_one_atom(aCurr, givenList, debug =0 ):
    ...
    '''
       ...
       '''
    close_atoms = get_close_atom_list(aCurr, givenList, debug=debug)

    if np.shape(close_atoms)[0] > 0:
        distInfo = get_close_atom_distance_info(aCurr, close_atoms, debug=debug)
    else:
        return []
...
def get_list_of_close_atoms_for_list_of_atoms(listOfInputAtoms,  aaType, debug = 0):
...
###########################Give me an atom, active atom Type, O/P: allCLOSE Atoms##########################################
        ...
        customList = stp.get_donor_acceptor_list(struct, aaType,debug=debug)

        #Get all the close atoms with the given custom list           
        allCloseAtoms = get_all_close_atom_info_for_one_atom(at, customList, debug=debug)
...
def get_hydrogen_connected_to_donor_backbone(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_ARG(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_ASH(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_GLH(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_ASN(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_GLN(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_TYR(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_TRP(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_HID(atomDonor, debug=0):
    '''
...
def get_hydrogen_connected_to_donor_HIE(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_HIP(atomDonor, debug=0):
    '''
    ...
    if(atomDonor.name == 'ND1'):
        atom3 = get_hydrogen_connected_to_donor_HID(atomDonor, debug=debug)
    
    if(atomDonor.name == 'NE2'):
        atom3 = get_hydrogen_connected_to_donor_HIE(atomDonor, debug=debug)
         
    return atom3


############################################SP3###################################################################3
def get_hydrogen_connected_to_donor_LYS(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_SER(atomDonor, debug=0):
...
def get_hydrogen_connected_to_donor_THR(atomDonor, debug=0):
...
def check_energy_range(energy, stateSet = False, debug=0):
    '''
    ...
    '''    
    if(abs(energy)>10):
        stp.append_to_log(f"Energy value found: {energy} is out of range. But the state is not yet set!\n", debug=0)
        if(stateSet ==True): 
            stp.append_to_log(f"Energy value: {energy} is out of range. Human intervention IS required. Exiting code \n", debug=0)
            os._exit(0)


def get_hydrogen_connected_to_donor(atomDonor,  debug=0):
    '''
    ...
    if(atomDonor.name == 'N'):
       atom3 = get_hydrogen_connected_to_donor_backbone(atomDonor, debug=debug)
       return atom3
    else: 
        ...
        try:
          ...
          stp.append_to_log(f"No ATOM3 for: {atomDonor.parent.resname} and {atomDonor.parent.id[1]} reason: either insufficent atoms or not found in dict \n", debug=0 )
          atom3 =[]
    return atom3
...
##############################COMPUTE ENERGY################################################################
    if(attractive==1):
        ...
        if(hPresent == -1): 
            stp.append_to_log(f"Donor atom:{atom2} belongs to res {atom2.parent} of chain: {atom2.parent.parent} has no hydrogen present \n", debug=0)

        else:
            atom3 = get_hydrogen_connected_to_donor(atom2, debug=debug) 
            #for various hydrogen atoms sticking out of donor (theta will change)
            try: atom3
            except NameError:
                stp.append_to_log(f"Insufficent atoms/PSEUDO LPs or info not found in any funcs \n", debug=0)
            ...
            for at in range(np.shape(atom3)[0]):
                ...
                check_energy_range(energy, debug=debug)
    ...
    #    #print(f"I am an ACCEPTOR being repulsive: {acceptorAt}, fake donor: {donorAt}, attractive :{attractive}")
        ...
        if(not LPNames):
            stp.append_to_log(f"##################################In compute_energy_as_acceptor, No Lone Pairs found for acceptor atom:{acceptorAt} of {acceptorAt.parent} and psAcceptor atom: {myPsAcceptor} of {myPsAcceptor.parent}, as attractive:{attractive} #################################", debug=0) 
...
########################### Compute Gamma and then energy!######################################################
    if(attractive ==1):
        ...
        if(not LPNames):
            stp.append_to_log(f"##########################In compute_energy_as_donor: No Lone Pairs found for acceptor atom:{acceptorAt} of {acceptorAt.parent} and donor atom: {donorAt} of {donorAt.parent} as attractive:{attractive} ################################", debug=0)
        ...
        if(hPresent ==-1):            
            stp.append_to_log(f"PS Donor(or acceptor) atom:{atom1} belongs res {atom1.parent} of its chain: {atom1.parent.parent} has NO hydrogen present", debug=0)
        else:
            ps_atom3 = get_hydrogen_connected_to_donor(atom1, debug=debug)
