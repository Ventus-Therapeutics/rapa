...
def compute_initial_position_for_atoms_connected_to_sp3(sp3Coord, surrCoord, bondLength, debug =0 ):
    ...
    '''
    #Find the direction
    sp3AAt0 = bondLength*mm.get_unit_vector(surrCoord-sp3Coord, debug=0) 
...
def create_prime_frame_of_reference(sp3Coord, aboveSp3Coord, aat0Coord, debug =0): 
    '''   
        Objective: 
    ...
    zHatPrime = mm.get_unit_vector(aboveSp3Coord - sp3Coord, debug=debug)
    ...
    xHatPrime = mm.get_unit_vector(xPrime, debug=debug)
    yHatPrime = np.cross(zHatPrime, xHatPrime)

    if(debug==1):
        stp.append_to_debug( __name__, sys._getframe().f_code.co_name, f"New frame of reference has unit vectors: {xHatPrime}, {yHatPrime}, {zHatPrime}!", debug=0)


    return xHatPrime, yHatPrime, zHatPrime


def setup_transformation_matrix( xHatPrime, yHatPrime, zHatPrime, debug=0):
...
def move_coords_to_prime_reference_frame(shiftByCoord, transformMatrix, allCoordsToMove, debug=0):
    ...
    translatedCoords = mm.translate_coord(shiftByCoord, allCoordsToMove, debug=debug)
...
def move_coords_to_original_reference_frame(shiftCoord, transformMatrix, moveCoordsBack, debug=0):
...
def get_all_connected_atoms(aat0CoordPrime, debug=0): 
    ...
    rotInPlane = np.array((mm.get_rotation_matrix(120, debug=debug),mm.get_rotation_matrix(240, debug=debug)))
...
def get_energy_of_all_close_atoms_for_hydrogen_lonepairs_connected_to_sp3(sp3, aboveSp3, aatPrime, allCloseAtoms, ang,
    ...
    if(debug == 1):
        stp.append_to_debug( __name__, sys._getframe().f_code.co_name,f"interaction energies for each close atom with respect to Hydrogen, Hydrogen, Hydrogen incase of LYS and Hydrogen, lone pair, lone pair incase of THR/SER. \
    \n {energyInteraction} ", debug=0) 

    return aat0Energy, aat1Energy, aat2Energy, enSum
#

def optimize_connected_atoms_by_rotation_in_plane(sp3, aboveSp3, aatPrime,allCloseAtoms, debug =0 ):
    '''
    ...
    for ind,ang in enumerate(range(0,endAng,10)):
        ...
        if(debug==1):
            ...
            check_bond_angle_for_all_connected_atoms(sp3, aboveSp3, sp3.coord, aboveSp3.coord, aatPrimeRot[0], aatPrimeRot[1], aatPrimeRot[2],debug=debug )
...
def compute_atoms_connected_to_sp3_in_prime_reference_frame(sp3, aboveSp3, allCloseAtoms, aat0CoordPrime, debug=0):
    ''' 
    ...
    aatPrime = get_all_connected_atoms(aat0CoordPrime, debug=debug) 


    if(debug==1):
        check_bond_angle_for_all_connected_atoms(sp3, aboveSp3, sp3.coord, aboveSp3.coord, aatPrime[0], aatPrime[1], aatPrime[2],debug=debug)
    #############################################################################
    ...
    S, aatPrimePositionMat  = optimize_connected_atoms_by_rotation_in_plane(sp3, aboveSp3, aatPrime, allCloseAtoms, debug=debug )
    ...
    if(all(x == 0 for x in S[:,-1].flatten())):
      ...
      stp.append_to_log("Taking h-bonding possibility at angle 0, as all energy values=0\n", debug=0)
...
def adjust_first_connected_atom_to_sp3_in_prime_reference_frame(aboveSp3CoordPrime, sp3CoordPrime, aat0CoordPrime, debug=0):
    ...
    aat0CoordPrimeMod = np.einsum('ij,j->i',mm.get_rotation_matrix_about_Yaxis(adjustTheta, debug=debug), aat0CoordPrime)
    
    return aat0CoordPrimeMod

def get_close_atom_coords(closeAtoms, debug=0):
...
def set_coords_in_prime_reference_frame(sp3, aboveSp3, allCloseAtoms, sp3CoordPrime, aboveSp3CoordPrime, closeAtomCoordsPrime, debug=0):
    ''' 
...
def set_coords_in_original_reference_frame(sp3, aboveSp3, allCloseAtoms, sp3Coord, aboveSp3Coord, allCloseAtomCoords, debug=0):
...
def print_close_atom_coords(closeAtoms, codePos="default", debug=0):
...
def compute_connected_atoms_to_sp3(sp3, aboveSp3, allCloseAtoms, debug=0):
     ...
     if(debug==1):
        ...
        fd.flush()
        fd.close()
        print_close_atom_coords(allCloseAtoms, codePos = "before computing initial attached atom coord:", debug=debug)
     aat0Coord = compute_initial_position_for_atoms_connected_to_sp3(sp3Coord, allCloseAtoms[1][0].coord, bondLen, debug=debug)

#    ##Create the new frame of reference
     xHatPrime, yHatPrime, zHatPrime = create_prime_frame_of_reference(sp3Coord, aboveSp3Coord, aat0Coord, debug=debug)
#    ##Create the transformation matrix
     transformMatrix = setup_transformation_matrix( xHatPrime, yHatPrime, zHatPrime, debug=debug)
     if(debug==1):
        fd = open(fDebugName, "a")
     ...
     primeCoords = move_coords_to_prime_reference_frame(sp3Coord, transformMatrix, moveCoords, debug=debug) 
...
#################### Move to new frame of reference###########################
     ...
     if(allCloseAtoms):
        closeAtomCoords = get_close_atom_coords(allCloseAtoms, debug=0)
        closeAtomCoordsPrime = move_coords_to_prime_reference_frame(sp3Coord, transformMatrix, closeAtomCoords, debug=debug)

     set_coords_in_prime_reference_frame(sp3, aboveSp3, allCloseAtoms, sp3CoordPrime, aboveSp3CoordPrime, closeAtomCoordsPrime, debug=debug)
##########################In the new frame ####################################################################
     aat0CoordPrimeMod = adjust_first_connected_atom_to_sp3_in_prime_reference_frame(aboveSp3.coord, sp3.coord, aat0CoordPrime, debug=debug)
     if(debug == True):
     ...
     aatPrimeDistMin, S, aatPrimeMat = compute_atoms_connected_to_sp3_in_prime_reference_frame(sp3, aboveSp3, allCloseAtoms, aat0CoordPrimeMod, debug=debug)

#########################End the new frame of reference####################################################################
     set_coords_in_original_reference_frame(sp3, aboveSp3, allCloseAtoms, sp3Coord, aboveSp3Coord, closeAtomCoords, debug=debug)
     if(debug==True):
        fd.write(f"back to original coords,sp3Coord: {sp3.coord}, above sp3Coord: {aboveSp3.coord}\n")
        fd.flush()
        fd.close()
        print_close_atom_coords(allCloseAtoms, codePos = "back to original frame of reference", debug=debug)
        check_bond_angle_for_all_connected_atoms( sp3, aboveSp3, sp3CoordPrime, aboveSp3CoordPrime, aatPrimeDistMin[0], aatPrimeDistMin[1], aatPrimeDistMin[2], debug=debug) 
        fd = open(fDebugName, "a")
#################### Get back to original frame of reference ONLY the MIN SUM DIST###########################
    
     numRows = np.shape(aatPrimeMat)[0]*np.shape(aatPrimeMat)[1]
     
     allAAt = move_coords_to_original_reference_frame(sp3Coord, transformMatrix, np.reshape(aatPrimeMat,(numRows,3)), debug =0) 
...
######Check if the optimized coordinate picked clashes with any surrounding atoms, if yes: pick next best option#######################
     for i,AAtInOriginal in reversed(list(enumerate(allAAtInOriginal))):
         ...
         for j in range(hMax):
             ...
             resSearchList  = stp.get_all_residues(structure, donotIncludeRes = residue, debug=debug)

             searchListAtoms = []
             for res in resSearchList:
                    searchListAtoms.append(stp.create_list_of_atoms_of_residue_without_lonepair(res, debug=debug))
...
def place_hydrogens_lonepairs_SER_THR(res, lastSerial, debug=0):
    ...
    customList = stp.get_known_donor_acceptor_list_for_one_atom(structure,sp3, aaType = 'DONOR_ACCEPTOR', debug=debug)
    ...
    allCloseAtoms = cats.get_all_close_atom_info_for_one_atom(sp3, customList, debug=debug)
    ...
    if(debug==1):
        fd.close()
    aatInOriginal, aatInOriginalAppended, coordAngEnergy = compute_connected_atoms_to_sp3(sp3, aboveSp3, allCloseAtoms, debug=debug)
...
def place_hydrogens_lonepairs_LYS(res, lastSerial, debug=0):
    '''
...
def add_sp3_side_chain_hydrogens_lonepairs(structure, lastSerial):

    ''' Objective: To add all SP3 Hydrogen SideChains '''

    res_dict = {'LYS': place_hydrogens_lonepairs_LYS, 'SER':place_hydrogens_lonepairs_SER_THR, 'THR': place_hydrogens_lonepairs_SER_THR}

    for res in structure.get_residues():
        try:
            lastSerial, hCoord = res_dict[res.resname](res, lastSerial)
        except KeyError:
            stp.append_to_log(f"Not SP3/insufficient Atoms: {res.resname} with ID: {res.id[1]} and chain: {res.parent}\n", debug=0 )
            pass
